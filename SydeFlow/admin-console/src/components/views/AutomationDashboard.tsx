...
export default function AutomationDashboard() {
  ...
  const executeWorkItem = async () => {
    if (!selectedBucket || !selectedObject || !selectedActivity) {
      ...
      toast.warning('Missing selection', 'Please select a bucket, file, and activity');
      return;
    }
    ...
    try {
      ...
      if (res.ok) {
        ...
        toast.updateToast(toastId, { type: 'info', title: 'Job submitted', message: 'Work item is now processing...' });
      } else {
        const error = await res.json();
        addLog('error', `WorkItem failed: ${error.diagnostic}`);
        setCurrentJob(prev => prev ? { ...prev, status: 'failed', error: error.diagnostic } : null);
        toast.updateToast(toastId, { type: 'error', title: 'Job failed', message: error.diagnostic });
      }
    } catch (err) {
      addLog('error', 'WorkItem execution failed');
      setCurrentJob(prev => prev ? { ...prev, status: 'failed', error: String(err) } : null);
      toast.updateToast(toastId, { type: 'error', title: 'Execution failed', message: String(err) });
    }
  };
            ...
            <div className="space-y-6">
              ...
              <div className="bg-slate-800 rounded-xl p-6">
                <h2 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
                  ...
                  Activity to Execute
                  <InfoTooltip 
                    content="Activities define what the Design Automation engine does with your file."
                    learnMoreUrl="https://aps.autodesk.com/en/docs/design-automation/v3/tutorials/revit/about_this_tutorial/"
                  />
                </h2>
                <select
                  ...
                  title="Select an activity to execute"
                >
                  <option value="">Select activity...</option>
                  {activities.filter(a => !a.toLowerCase().includes('extractparam')).map(a => (
              ...
              <details className="bg-slate-800 rounded-xl overflow-hidden group">
                <summary className="p-4 cursor-pointer flex items-center justify-between hover:bg-slate-700/50 transition-colors">
                  <div className="flex items-center gap-2">
                    ...
                    <InfoTooltip 
                      content="App bundles contain your custom plugin code that runs in the cloud."
                      learnMoreUrl="https://aps.autodesk.com/en/docs/design-automation/v3/tutorials/revit/step2-create-appbundle/"
                    />
                  </div>
