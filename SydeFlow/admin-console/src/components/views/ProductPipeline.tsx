...
export default function ProductPipeline() {
  ...
  const [currentStep, setCurrentStep] =
    useState<PipelineStep>("product-details");
  ...
  const [propExtractionStatus, setPropExtractionStatus] = useState<string | null>(null);
  const [extractingProps, setExtractingProps] = useState(false);
  const [extractionStatus, setExtractionStatus] = useState("");

  // Extraction setup state (APS Design Automation bundle/activity)
  const [extractionSetupReady, setExtractionSetupReady] = useState<
    boolean | null
  >(null);
  ...
  const [fileSource, setFileSource] = useState<"oss" | "local">("oss");
  const [localFile, setLocalFile] = useState<File | null>(null);
  const [uploading, setUploading] = useState(false);
  const [uploadProgress, setUploadProgress] = useState(0);
  const [uploadedFileUrn, setUploadedFileUrn] = useState<string | null>(null);
  const [translationStatus, setTranslationStatus] = useState<
    "none" | "pending" | "inprogress" | "success" | "failed"
  >("none");
  ...
  useEffect(() => {
    ...
    socketRef.current.on(
      ...
      (data: { status: string; progress?: string }) => {
        setExtractionStatus(
          `Extracting: ${data.status} ${data.progress || ""}`,
        );
      },
    );
    ...
    socketRef.current.on(
      "extractionComplete",
      (data: { success: boolean; parameters?: any[]; error?: string }) => {
        setExtracting(false);
        if (data.success && data.parameters) {
          setExtractionStatus(`Found ${data.parameters.length} parameters!`);
          // Convert to ParameterConfig format
          const paramConfigs: ParameterConfig[] = data.parameters.map(
            (p: any) => ({
              inventorName: p.name,
  ...
  const loadActivities = async () => {
    ...
    try {
      const res = await apiFetch("/api/aps/designautomation/activities");
      if (res.ok) {
        const data = await res.json();
        setActivities(data || []);
      }
  ...
  const loadObjects = async (bucketKey: string) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const inventorFiles = allObjects.filter(
          (obj: any) =>
  ...
  const checkExtractionSetup = async () => {
    try {
      const res = await apiFetch("/api/extract-parameters/status");
      if (res.ok) {
        const data = await res.json();
  ...
  const setupExtractionAutomation = async () => {
    ...
    try {
      const res = await apiFetch("/api/extract-parameters/setup", {
        method: "POST",
  ...
  const handleLocalFileUpload = async () => {
    ...
    try {
      ...
      formData.append("bucketKey", selectedBucket);
      ...
      if (result.success || result.objectKey) {
        // Update product with uploaded file info
        const uploadBucket = result.bucketKey || selectedBucket;
        const objectKey = result.objectKey || localFile.name;

        setProduct((prev) => ({
          ...prev,
          ossBucket: uploadBucket,
          sourceFile: {
        ...
        // Update selected bucket to match upload destination
        setSelectedBucket(uploadBucket);

        // Refresh file list
        loadObjects(uploadBucket);

        // Store URN for translation status tracking
        if (result.urn) {
          setUploadedFileUrn(result.urn);
        ...
        // Switch to OSS tab to show the uploaded file
        setFileSource("oss");
      }
    } catch (err) {
  ...
  const pollTranslationStatus = async (urn: string) => {
    ...
    const checkStatus = async () => {
      try {
        const res = await apiFetch(`/api/translation/${urn}`);
        const data = await res.json();
        ...
        console.error("Translation status check failed:", err);
        attempts++;
        if (attempts < maxAttempts) {
  ...
  const extractParameters = async () => {
    ...
    try {
      // Check if Design Automation extraction is set up
      if (extractionSetupReady) {
        // Use Design Automation extraction (required for Inventor parameters)
        setExtractionStatus("Running Inventor to extract parameters...");
        const res = await apiFetch("/api/extract-parameters", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ossBucket: bucketKey,
        ...
        if (res.ok && data.status === "started") {
          ...
          setExtractionStatus(
            "Extraction running in Inventor... waiting for results",
          );
        } else {
          setExtractionStatus(
            `Failed: ${data.diagnostic || data.message || "Unknown error"}`,
          );
          setExtracting(false);
        }
      } else {
        // Try simple extraction as fallback (works for some file types)
        const res = await apiFetch("/api/extract-parameters-simple", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ossBucket: bucketKey,
        ...
        if (res.ok && data.success && data.parameters?.length > 0) {
          // Simple extraction worked - update parameters directly
          setExtractionStatus(
            `Found ${data.parameters.length} parameters from ${data.source || "file"}`,
          );
          setProduct((prev) => ({
            ...prev,
          ...
          // Simple extraction found nothing - need DA setup
          setExtractionStatus(
            "No parameters found. For Inventor files, set up Design Automation extraction first.",
          );
          setExtracting(false);
        }
      }
...
const extractProperties = async () => {
    ...
    setExtractingProps(true);
    setPropExtractionStatus("Starting property extraction...");

    try {
      // Check if Design Automation extraction is set up
      if (extractionSetupReady) {
        // Use Design Automation extraction (required for Inventor properties)
        setPropExtractionStatus("Running Inventor to extract properties...");
        const res = await apiFetch("/api/extract-properties", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ossBucket: bucketKey,
        ...
        if (res.ok && data.status === "started") {
          ...
          setExtractingProps(false);
        }
      } else {
        // Try simple extraction as fallback (works for some file types)
        const res = await apiFetch("/api/extract-properties-simple", {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
            ossBucket: bucketKey,
        ...
        if (res.ok && data.success && data.properties?.length > 0) {
          // Simple extraction worked - update properties directly
          setPropExtractionStatus(
            `Found ${data.properties.length} properties from ${data.source || "file"}`,
          );
          setProduct((prev) => ({
            ...prev,
          ...
          // Simple extraction found nothing - need DA setup
          setPropExtractionStatus(
            "No properties found. For Inventor files, set up Design Automation extraction first.",
          );
          setExtractingProps(false);
        }
      }
    } catch (err) {
      console.error("Failed to extract properties:", err);
      setPropExtractionStatus("Failed to start extraction");
      setExtractingProps(false);
    }
  };
  ...
  const updateProductStatus = async (
    id: string,
    newStatus: "draft" | "testing" | "published",
  ) => {
    try {
  ...
  const renderFileUpload = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mb-6">
          Select the Inventor file (.ipt or .iam) that will be used as the base
          for this configurable product.
        </p>
      </div>
        ...
        <div className="space-y-4">
          ...
          <div className="p-4 bg-slate-700/50 rounded-lg">
            ...
            <select
              ...
              className="w-full px-4 py-2 bg-slate-700 border border-slate-600 rounded-lg text-white focus:ring-2 focus:ring-orange-500 focus:border-orange-500"
              disabled={loadingBuckets}
              title="Upload Destination Bucket"
            >
              <option value="">Select bucket...</option>
              {buckets.map((b) => (
                <option key={b.bucketKey} value={b.bucketKey}>
            ...
            {!selectedBucket && (
              <p className="mt-2 text-xs text-yellow-400">
                Please select a bucket before uploading
              </p>
            )}
          </div>
          ...
          >
            <input
              ref={fileInputRef}
              type="file"
              accept=".ipt,.iam,.idw,.ipn,.dwg"
              onChange={handleLocalFileSelect}
              className="hidden"
              title="Select CAD file"
              aria-label="Select CAD file to upload"
            />
            ...
            {localFile ? (
              <div className="flex flex-col items-center gap-3">
                ...
                <div>
                  <p className="font-medium text-green-300">{localFile.name}</p>
              ...
              <div className="flex flex-col items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-gray-500">
                    or click to browse (.ipt, .iam, .idw, .ipn, .dwg)
                  </p>
                </div>
              </div>
            )}
          ...
          {localFile && !uploading && (
            <button
              ...
              className="w-full flex items-center justify-center gap-2 px-4 py-3 bg-gradient-to-r from-orange-500 to-orange-600 text-white rounded-lg hover:from-orange-600 hover:to-orange-700 disabled:opacity-50 disabled:cursor-not-allowed font-medium transition-all"
            >
      ...
      {product.sourceFile?.objectKey && (
        <div className="p-4 bg-green-500/10 border border-green-500/30 rounded-lg">
          ...
          {/* Tip about parameter extraction */}
          <div className="mt-3 pt-3 border-t border-green-500/20">
            <p className="text-xs text-gray-400">
              <span className="text-orange-400">Tip:</span> Parameter extraction
              uses Design Automation and works immediately - no need to wait for
              translation.
            </p>
          </div>
        </div>
  ...
  const renderParameterConfig = () => (
    <div className="space-y-6">
      {/* Extraction Setup Status Banner - show when NOT ready or still checking */}
        ...
        <div className="p-4 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="font-medium text-yellow-300">
                  {extractionSetupReady === null
                    ? "Checking Setup Status..."
                    : "Extraction Setup Required"}
                </p>
                <p className="text-sm text-yellow-400/70">
                  {extractionSetupReady === null
                    ? "Verifying if parameter extraction automation is configured."
            ...
            <div className="flex gap-2">
              <button
                onClick={checkExtractionSetup}
                disabled={settingUpExtraction}
                className="flex items-center gap-2 px-3 py-2 bg-gray-600 text-white rounded-lg hover:bg-gray-500 disabled:opacity-50"
                title="Check status"
              >
          ...
          <div className="flex items-center justify-between">
            ...
            <button
              onClick={async () => {
                if (
                  confirm(
                    "Reset extraction automation? This will delete the bundle and activity from APS.",
                  )
                ) {
                  setSettingUpExtraction(true);
                  setExtractionStatus("Resetting extraction automation...");
                  try {
                    const res = await apiFetch("/api/extract-parameters/reset", {
                      method: "DELETE",
                    });
                    const data = await res.json();
                    if (res.ok) {
                      setExtractionSetupReady(false);
                      setExtractionStatus(
                        "Reset complete. Click Auto Setup to reconfigure.",
                      );
                    } else {
                      setExtractionStatus(
                        `Reset failed: ${data.diagnostic || "Unknown error"}`,
                      );
                    }
                  } catch (err) {
                    setExtractionStatus(
                      "Reset failed - check server connection",
                    );
                  }
                  setSettingUpExtraction(false);
                }
      ...
      {propExtractionStatus && (
        <div
          className={`p-3 rounded-lg text-sm ${
            propExtractionStatus.includes("Failed") ||
            propExtractionStatus.includes("failed")
              ? "bg-red-500/10 border border-red-500/30 text-red-400"
              : propExtractionStatus.includes("Found") ||
                  propExtractionStatus.includes("✓")
                ? "bg-green-500/10 border border-green-500/30 text-green-400"
                : "bg-blue-500/10 border border-blue-500/30 text-blue-400"
          }`}
        >
      ...
      {product.properties && Object.keys(product.properties).length > 0 && (
        <div className="mb-6 p-4 bg-slate-800/50 rounded-lg border border-slate-600">
          <h4 className="text-sm font-semibold text-gray-300 mb-3 flex items-center gap-2">
            <Database className="w-4 h-4 text-blue-400" /> Model Properties
          </h4>
          <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
            {product.properties.physicalProperties && (
              <>
                <div className="bg-slate-900/50 p-2 rounded border border-slate-700">
                  <span className="text-xs text-gray-500 block">Mass</span>
                  <span className="text-sm text-gray-200">
                    {product.properties.physicalProperties.mass?.value?.toFixed(3)} {product.properties.physicalProperties.mass?.unit}
                  </span>
                </div>
                <div className="bg-slate-900/50 p-2 rounded border border-slate-700">
                  <span className="text-xs text-gray-500 block">Volume</span>
                  <span className="text-sm text-gray-200">
                    {product.properties.physicalProperties.volume?.value?.toFixed(3)} {product.properties.physicalProperties.volume?.unit}
                  </span>
                </div>
                <div className="bg-slate-900/50 p-2 rounded border border-slate-700">
                  <span className="text-xs text-gray-500 block">Area</span>
                  <span className="text-sm text-gray-200">
                    {product.properties.physicalProperties.area?.value?.toFixed(3)} {product.properties.physicalProperties.area?.unit}
                  </span>
                </div>
              </>
            )}
            {product.properties.iProperties && Object.entries(product.properties.iProperties).map(([key, val]) => (
          ...
          {product.parameters?.map((param, index) => (
            ...
            >
              <div className="flex items-start gap-4">
                ...
                <button
                  onClick={() =>
                    updateParameter(index, { exposed: !param.exposed })
                  }
                ...
                <div className="flex-1 grid grid-cols-4 gap-4">
                  <div>
                    ...
                    <input
                      ...
                      onChange={(e) =>
                        updateParameter(index, { displayName: e.target.value })
                      }
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateParameter(index, {
                          controlType: e.target.value as any,
                        })
                      }
                      className="w-full px-3 py-1.5 text-sm bg-slate-700 border border-slate-600 rounded text-white"
                      title="Control Type"
                  ...
                  <div>
                    ...
                    <input
                      type={
                        param.controlType === "checkbox" ? "checkbox" : "text"
                      }
  ...
  const renderAutomationSetup = () => (
    <div className="space-y-6">
      <div>
        ...
        <p className="text-sm text-gray-400 mb-6">
          Configure how the product will be processed when customers place
          orders.
        </p>
      </div>
      ...
      <div>
        ...
        <div className="flex flex-wrap gap-2">
          {["step", "pdf", "dwg", "iges"].map((format) => (
            <label
              key={format}
              className="flex items-center gap-2 px-3 py-2 bg-slate-700 border border-slate-600 rounded-lg cursor-pointer hover:bg-slate-600"
            >
              <input
                type="checkbox"
                checked={product.automation?.additionalOutputs?.includes(
                  format,
                )}
                onChange={(e) => {
              ...
              .map((param, index) => (
                ...
                >
                  ...
                  <select
                    ...
                    onChange={(e) => {
                      const paramIndex = product.parameters?.findIndex(
                        (p) => p.inventorName === param.inventorName,
                      );
                  ...
                  {param.priceModifier?.type &&
                    param.priceModifier.type !== "none" && (
                      <input
                        type="number"
      ...
      <div className="grid grid-cols-2 gap-6">
            ...
            <dl className="space-y-2 text-sm">
              ...
              {product.pricing?.setupFee && (
                <div className="flex justify-between">
                  ...
                  <dd className="font-medium text-white">
                    ${product.pricing.setupFee.toFixed(2)}
                  </dd>
                </div>
              )}
            </dl>
        ...
        <div className="p-4 bg-slate-800/50 rounded-lg border border-slate-700">
          <h4 className="font-medium text-white mb-3">
            Customer Parameters (
            {product.parameters?.filter((p) => p.exposed).length || 0})
          </h4>
      ...
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-gray-400">
            Create and manage configurable products with automated workflows
          </p>
        </div>
        <button
              ...
              >
                ...
                <div className="p-4 border-b border-slate-700/50">
                  <div className="flex items-start justify-between mb-2">
                    ...
                    <span
                      className={`flex items-center gap-1 px-2 py-1 rounded-full text-xs font-medium border ${statusColors[prod.status] || statusColors.draft}`}
                    >
                  ...
                  {prod.pricing && (
                    <div className="flex items-center justify-between text-sm">
                      ...
                      <span className="text-green-400 font-medium">
                        ${prod.pricing.basePrice?.toFixed(2)}
                      </span>
                    </div>
                  )}
                  <div className="flex items-center justify-between text-sm">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateProductStatus(
                          prod.id,
                          e.target.value as "draft" | "testing" | "published",
                        )
                      }
