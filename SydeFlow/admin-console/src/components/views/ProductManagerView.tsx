...
export default function ProductManagerView() {
  ...
  const [propExtractionStatus, setPropExtractionStatus] = useState<string>("");
  ...
  useEffect(() => {
    ...
    socketRef.current.on("propExtractionComplete", (data: any) => {
      ...
      if (data.success && data.properties) {
        setPropExtractionStatus("Found model properties!");
        setFormData((prev) => ({ ...prev, properties: data.properties }));
      } else {
        setPropExtractionStatus(`Extraction failed: ${data.error || "Unknown error"}`);
      }
    });
  ...
  const extractParameters = async (useSimple: boolean = false) => {
    if (!formData.ossBucket || !formData.ossObjectKey) {
      toast.warning("Please select an OSS bucket and file first");
      return;
    }
    ...
    try {
      ...
      if (useSimple && data.success) {
        setExtractionStatus(
          `Found ${data.parameters.length} parameters from ${data.source}`,
        );
        setFormData((prev) => ({
          ...prev,
          parameters: data.parameters,
  ...
  // Extract properties from Inventor file
  const extractProperties = async (useSimple: boolean = false) => {
    ...
    try {
      const endpoint = useSimple
        ? "/api/extract-properties-simple"
        : "/api/extract-properties";
      const response = await fetch(endpoint, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
      ...
      if (!response.ok) {
        if (data.hint) {
          setPropExtractionStatus(`${data.diagnostic}\n${data.hint}`);
        } else {
          setPropExtractionStatus(`Error: ${data.diagnostic}`);
        }
        setExtractingProps(false);
        return;
      }
      ...
      if (useSimple && data.success) {
        setPropExtractionStatus(
          `Found ${Object.keys(data.properties || {}).length} properties from ${data.source}`,
        );
        setFormData((prev) => ({
          ...prev,
        ...
        setPropExtractionStatus(data.message || "No properties found");
        setExtractingProps(false);
      } else {
        // Full extraction uses socket for progress
        setPropExtractionStatus(data.message || "Extraction started...");
      }
    } catch (ex: any) {
      setPropExtractionStatus(`Error: ${ex.message}`);
      setExtractingProps(false);
    }
  };
  ...
  const downloadExtractionTemplate = async () => {
    try {
      ...
      // Show instructions
      toast.success("iLogic template downloaded! Check the instructions panel below.");
    } catch (ex: any) {
      toast.error("Failed to download template: " + (ex as any).message);
    }
  };
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Products List */}
      {products.length === 0 ? (
        <EmptyState type="products" onAction={openCreateModal} />
      ) : (
        <div className="space-y-4">
          {Object.entries(productsByCategory).map(
            ([categoryId, categoryProducts]) => {
              ...
              const IconComponent =
                categoryIcons[category?.icon || "Box"] || Box;
                        ...
                        >
                          <div className="flex items-center justify-between">
                            ...
                            <div className="flex items-center gap-3">
                              {/* Status Badge */}
                              <div
                                className={`flex items-center gap-1.5 px-3 py-1 rounded-full text-xs font-medium border ${statusColor}`}
                              >
                              ...
                              <a
                                ...
                                className="p-2 hover:bg-slate-600/50 rounded-lg transition-colors text-green-400"
                              ...
                              <button
                                onClick={() => setDeleteConfirm({ open: true, productId: product.id, productName: product.name })}
                ...
                <div className="space-y-4">
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      >
                        <RefreshCw
                          className={`w-5 h-5 text-gray-400 ${loadingBuckets ? "animate-spin" : ""}`}
                        />
                      </button>
                    </div>
                  </div>
                  <div>
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={() =>
                          formData.ossBucket &&
                          fetchBucketObjects(formData.ossBucket)
                        }
                        className="p-2 bg-slate-700 hover:bg-slate-600 rounded-lg transition-colors flex-shrink-0"
                        title="Refresh files"
                        disabled={!formData.ossBucket}
                      >
                        <RefreshCw
                          className={`w-5 h-5 text-gray-400 ${loadingObjects ? "animate-spin" : ""}`}
                        />
                      </button>
                    </div>
                  </div>
              ...
              <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    aria-label="Design Automation activity"
                  >
                  ...
                  >
                    <RefreshCw
                      className={`w-5 h-5 text-gray-400 ${loadingActivities ? "animate-spin" : ""}`}
                    />
                  </button>
                </div>
                <p className="text-xs text-gray-500 mt-2">
                  Select the Design Automation activity that will process this
              ...
              <div className="bg-slate-800/50 rounded-xl p-4 border border-slate-700/50">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <button
                      onClick={() => extractProperties(true)}
                      disabled={
                        !formData.ossBucket ||
                        !formData.ossObjectKey ||
                        extractingProps
                      }
                      className="flex items-center gap-1 px-3 py-1 bg-cyan-500/20 hover:bg-cyan-500/30 text-cyan-400 rounded-lg text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                    ...
                    <button
                      onClick={() => extractProperties(false)}
                      disabled={
                        !formData.ossBucket ||
                        !formData.ossObjectKey ||
                        extractingProps
                      }
                      className="flex items-center gap-1 px-3 py-1 bg-teal-500/20 hover:bg-teal-500/30 text-teal-400 rounded-lg text-sm transition-colors disabled:opacity-50 disabled:cursor-not-allowed"
                      title="DA Property Extraction using APS Design Automation"
                    >
                      {extractingProps ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
                      ) : (
                ...
                {propExtractionStatus && (
                  <div
                    className={`mb-4 p-3 rounded-lg text-sm ${
                      propExtractionStatus.includes("Error") ||
                ...
                {(formData as any).properties &&
                  Object.keys((formData as any).properties).length > 0 && (
                    <div className="mb-4 p-4 bg-slate-900/50 rounded-lg border border-teal-700/30">
                      <h5 className="text-sm font-semibold text-teal-400 mb-3 flex items-center gap-2">
                        <Database className="w-4 h-4" />
                        Model Properties
                      </h5>
                      <div className="grid grid-cols-2 md:grid-cols-3 gap-3">
                        {(formData as any).properties.physicalProperties && (
                          <>
                            {(formData as any).properties.physicalProperties.mass && (
                              <div className="bg-slate-800 p-2 rounded border border-slate-700">
                                <span className="text-xs text-gray-500 block">Mass</span>
                                <span className="text-sm text-gray-200">
                                  {(formData as any).properties.physicalProperties.mass.value?.toFixed(3)}{" "}
                                  {(formData as any).properties.physicalProperties.mass.unit}
                                </span>
                              </div>
                            )}
                            {(formData as any).properties.physicalProperties.volume && (
                              <div className="bg-slate-800 p-2 rounded border border-slate-700">
                                <span className="text-xs text-gray-500 block">Volume</span>
                                <span className="text-sm text-gray-200">
                                  {(formData as any).properties.physicalProperties.volume.value?.toFixed(3)}{" "}
                                  {(formData as any).properties.physicalProperties.volume.unit}
                                </span>
                              </div>
                            )}
                            {(formData as any).properties.physicalProperties.area && (
                              <div className="bg-slate-800 p-2 rounded border border-slate-700">
                                <span className="text-xs text-gray-500 block">Area</span>
                                <span className="text-sm text-gray-200">
                                  {(formData as any).properties.physicalProperties.area.value?.toFixed(3)}{" "}
                                  {(formData as any).properties.physicalProperties.area.unit}
                                </span>
                              </div>
                            )}
                          </>
                        )}
                        {(formData as any).properties.iProperties &&
                          Object.entries(
                            (formData as any).properties.iProperties,
                          ).map(([key, val]) => (
                            <div
                ...
                {formData.parameters?.length === 0 ? (
                  <div className="text-center py-6">
                    ...
                    <p className="text-gray-600 text-xs">
                      Select a file above, then use "Quick Extract" to
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-slate-900 rounded-2xl shadow-2xl w-full max-w-5xl max-h-[90vh] overflow-hidden border border-slate-700">
            <div className="p-6 border-b border-slate-700/50 flex items-center justify-between">
              <div>
                <h3 className="text-xl font-bold text-white">
                  Test: {selectedProduct.name}
                </h3>
                <p className="text-sm text-gray-400">
                  Configure parameters and run Design Automation
                </p>
              </div>
              <button
              ...
              <div className="w-80 border-r border-slate-700/50 p-4 overflow-y-auto">
                            ...
                            {param.min !== undefined &&
                              param.max !== undefined && (
                                <input
                                  ...
                                  aria-label={`${param.displayName || param.name} slider`}
                                />
                              )}
                          </div>
                        )}
                ...
                <div className="mt-6 space-y-3">
                  <button
                    ...
                    disabled={
                      testRunning || selectedProduct.parameters.length === 0
                    }
                ...
                {testUrn ? (
                  ...
                  <div className="text-center">
                    ...
                    <p className="text-gray-500">
                      Configure parameters and run a test
                    </p>
      ...
      <ConfirmDialog
        ...
        title={`Delete "${deleteConfirm.productName}"?`}
        message="This will permanently delete this product and all its associated data. This action cannot be undone."
        confirmLabel="Delete"
        onConfirm={() => {
          handleDeleteProduct(deleteConfirm.productId);
          setDeleteConfirm({ open: false, productId: '', productName: '' });
        }}
        onCancel={() => setDeleteConfirm({ open: false, productId: '', productName: '' })}
      />
    </div>
  );
}
