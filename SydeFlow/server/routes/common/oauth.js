...
// Track last known credentials to detect changes
let lastCredentialsHash = '';

// APS connection status
let apsConnectionStatus = {
    connected: false,
    lastCheck: null,
    error: null
};
...
/**
 * Clears the OAuth cache (call when credentials change)
 */
function clearCache() {
    cache = {};
    apsConnectionStatus = {
...
/**
 * Gets current APS connection status
 */
...
/**
 * Checks if credentials have changed
 */
function credentialsChanged() {
    const { client_id, client_secret } = config.credentials;
    const currentHash = `${client_id || ''}:${client_secret || ''}`;
    if (currentHash !== lastCredentialsHash) {
        lastCredentialsHash = currentHash;
        return true;
    }
    return false;
}
...
async function getClient(scopes) {
    // Check if credentials changed, clear cache if so
    ...
    try {
        ...
        if (!client_id || !client_secret) {
            console.error('Missing APS credentials! client_id:', client_id ? 'SET' : 'MISSING', 'client_secret:', client_secret ? 'SET' : 'MISSING');
            apsConnectionStatus = {
                connected: false,
        ...
        console.error('OAuth authentication failed:', ex.message);
        apsConnectionStatus = {
            connected: false,
            lastCheck: new Date().toISOString(),
            error: ex.message
...
/**
 * Gets internal (2-legged) token for server-side operations
 * Returns an object with oauth client and credentials
 * @param {string[]} scopes Optional scopes
 * @returns {Promise<{client: AuthClientTwoLegged, credentials: object}>}
 */
async function getInternalToken(scopes) {
    const client = await getClient(scopes || config.scopes.internal);
    if (!client) {
        throw new Error('Failed to get OAuth client - check credentials');
    }
    return {
        ...client,
        client,
