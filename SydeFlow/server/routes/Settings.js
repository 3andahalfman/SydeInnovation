...
// GET /api/settings/reveal-secret - Get the actual secret (admin only, already auth-gated)
router.get('/reveal-secret', (req, res) => {
    ...
    // Return only last 4 chars for verification
    const secret = settings.apsClientSecret;
    res.json({ 
        success: true, 
...
// GET /api/settings/env-status - Check environment variable status (admin only)
